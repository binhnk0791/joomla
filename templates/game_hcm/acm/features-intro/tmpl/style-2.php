			...
			<?php for ($i=0; $i<$count; $i++) : ?>
				...
				<div class="features-item col-sm-<?php echo $column ?>">
					<?php if($helper->get('data.img-icon', $i)) : ?>
						<a class="img-icon" href="<?php if($helper->get('data.url', $i)) : ?><?php echo $helper->get('data.url', $i) ?><?php endif ; ?>" target="_blank">
							<img src="<?php echo $helper->get('data.img-icon', $i) ?>" alt="<?php if($helper->get('data.title', $i)) : ?><?php echo $helper->get('data.title', $i) ?><?php endif ; ?>" />
						</a>
					<?php endif ; ?>
